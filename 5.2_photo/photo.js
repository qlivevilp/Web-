...
//单击事件 reset()方法，在该方法中调用 initCanvas()方法，每次在不同的位置绘制圆形区域。
function reset(){
    initCanvas();
}
//单击事件 show()方法，在该方法中调用 draw(image)方法，使圆形半径大于画布，这时就可以绘制完整的图片了，也就是收到红包的效果。*/
function show(){
